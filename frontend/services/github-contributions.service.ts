...
import { RepositoryContributions, ContributionStats } from "@/@types/github";
...
export async function getUserContributionStats(
	...
	try {
		...
		const userPRs = contributions.pullRequests.filter((pr) => pr.author === username);
		const userIssues = contributions.issues.filter((issue) => issue.author === username);
